...
public class SQL2Neo4J {
	...
	public static void main(String[] args) throws Exception {
		...
		GraphDatabaseService graphDataService = new GraphDatabaseFactory()
				.newEmbeddedDatabase(Neo4j_Path);
		...
		// Map<String,List<String>> columns = new
		...
		Node data = neo.createUniqueFactory(database, "database", "Database",
				graphDataService);
		...
		for (int t = 0; t < tables.size(); t++) {
			...
			// table--database
			Node table = neo.createNode(tables.get(t), "table", "Table",
					database, graphDataService);
			...
			for (String column : columns) {
				...
				Node col = neo.createNode(column, "column", "Column", parent,
						graphDataService);
				currCols.add(col);
				...
				for (String record : records) {
					Node cell = neo.createUniqueFactory(record, "record",
							"Record", graphDataService);
		...
		try (Transaction trans = graphDataService.beginTx()) {
			...
			for (Node reeve : currTables) {
				HashSet<FK> fks = test.getFK(reeve.getProperty("value")
						.toString(), database);
				if (fks != null) {
					for (FK fk : fks) {
						...
						result = engine.execute("MATCH (n) where n.value='"
								+ column + "' and n.parent='" + parent
								+ "' RETURN n");
						Node foreignK = null;
						...
						result = engine.execute("MATCH (n) where n.value='"
								+ rcolumn + "' and n.parent='" + rparent
								+ "' RETURN n");
						Node primaryK = null;
						for (Map<String, Object> temp : result) {
							primaryK = (Node) temp.get("n");
						}
						neo.createRel(primaryK, foreignK, "PK-FK",
								graphDataService);

						// link table
						String rel = database + "." + table + "." + column
								+ "=" + database + "." + rtable + "." + rcolumn;
						result = engine.execute("MATCH (n) where n.value='"
								+ table + "' and n.parent='" + database
								+ "' RETURN n");
						Node table1 = null;
						for (Map<String, Object> temp : result) {
			...
			for (Node currCol : currCols) {
				...
				result = engine.execute("MATCH (n:`Column`) where n.value='"
						+ value + "' RETURN n");
				for (Map<String, Object> temp : result) {
					Node col = (Node) temp.get("n");
					String[] db = currCol.getProperty("parent").toString()
							.split("-");
					String[] rdb = col.getProperty("parent").toString()
							.split("-");
					if (!col.equals(currCol) && !db[1].equals(rdb[1])) {
						neo.createRel(currCol, col, "SameNameCol",
								graphDataService);

						result = engine.execute("MATCH (n) where n.value='"
								+ db[0] + "' and n.parent='" + db[1]
								+ "' RETURN n");
						Node table1 = null;
						...
						result = engine.execute("MATCH (n) where n.value='"
								+ rdb[0] + "' and n.parent='" + rdb[1]
								+ "' RETURN n");
						Node table2 = null;
						for (Map<String, Object> tem : result) {
							table2 = (Node) tem.get("n");
						}
						String rel = db[1] + "." + db[0] + "." + value + "="
								+ rdb[1] + "." + rdb[0] + "."
								+ col.getProperty("value");
		...
		// table---database
		/*
		 * int n=0; while(n<tables.size()){ Node
		 * table=neo.createNode(tables.get(n), "table", "Table", database,
		 * graphDataService); //Node
		 * table=neo.createUniqueFactory(tables.get(n), "table",
		 * "Table",graphDataService); relation.add(neo.createRel(table, data,
		 * "value-record", graphDataService)); n++;
		 * 
		 * }
		 */

		/*
		 * int i=0; while(i<tables.size()){ columns.put(tables.get(i),
		 * test.getColumnName(tables.get(i))); i++;
		 * //System.out.println(columns); } Label label = DynamicLabel.label(
		 * "Table" );
		 * 
		 ...
		 * int k=0;
		 * 
		 * Node col=neo.createNode(val, "column", "Column", key,
		 * graphDataService); //Node tab=neo.createNode(key, "table", "Table",
		 * database, graphDataService);
		 * 
		 * Node tab=neo.getNode(key, label, database, graphDataService); //Node
		 * col=neo.createUniqueFactory(val, "column","Column",graphDataService);
		 * //Node tab=neo.createUniqueFactory(key,
		 * "table","Table",graphDataService); relation.add(neo.createRel(col,
		 * tab, "column-table", graphDataService));
		 * //System.out.println("-----------");
		 * 
		 * values=test.getValue(val, key); //System.out.println(values);
		 * 
		 * while(k<values.size()){
		 * 
		 * Node record=neo.createUniqueFactory(values.get(k),
		 * "record","Record",graphDataService);
		 * relation.add(neo.createRel(record, col, "value-record",
		 * graphDataService)); k++;
		 * 
		 * }
		 */
